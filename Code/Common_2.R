# This file contains the common first steps for every code, but applied to RNAseq data: loading the data, filtering exp so that we only have the IDH-WT primary tumors & the expression of the first 15 genes. Note that the data is internally normalized. 

# First load the data

pheno <- read.delim('Data/Pheno_RNA-Seq.txt') # Contains phenotypic information of the samples
exp <- read.delim('Data/TCGA_RNASeq', row.names = 1) # Measures expression levels for different genes in each sample
